...
public abstract class Bag {
    ...
    public boolean addItem(String item) {
        if (this.numberOfContents < this.capacity) {
            ...
            // load the new array with items already in the bag
            System.arraycopy(this.contents, 0, newarr, 0, n);

            // add the new item to the new array
            newarr[n] = item;

            // assign contents to the new array
     ...
     *
     * @return String itemToBeRemoved
     */
    public String popItem() {
        if (this.numberOfContents == 0) {
            return null;
        } else {
            String itemToBeRemoved = this.contents[this.contents.length-1];
            int n = this.contents.length;
            String[] newar = new String[n - 1];

            System.arraycopy(this.contents, 0, newar, 0, n - 1);
            this.contents = newar;
            this.numberOfContents--;
